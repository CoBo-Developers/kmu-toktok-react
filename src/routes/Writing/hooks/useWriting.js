...
const useWriting = (writingId) => {
  ...
  const [writingList] = useWritingStore((state) => [
    state.writingList,
  ...
  useEffect(() => {
    ...
    getWriting(cookies.accessToken, writingId)
      .then((res) => {
        ...
        const now = new Date();
        if (
          parseDateString(assignment.startDate) <= now &&
          parseDateString(assignment.endDate).setDate(parseDateString(assignment.endDate).getDate() + 1) >= now
        ) {
          setIsExpired(false);
        } else {
          setIsExpired(true);
